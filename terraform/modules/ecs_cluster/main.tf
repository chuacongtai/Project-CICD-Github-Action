resource "aws_ecs_cluster" "ecs_cluster_blue" {
  name = "cluster-blue"
  
}
resource "aws_ecs_cluster" "ecs_cluster_green" {
  name = "cluster-green"
...
resource "aws_ecs_service" "nodejs_service_blue" {
  name            = "nodejs-service"
  ...
  cluster         = aws_ecs_cluster.ecs_cluster_blue.id
  ...
  load_balancer {
    target_group_arn = var.nodejs_target_group_arn_blue
    container_name   = "nodejs-container"
    container_port   = 3000
  }

}
resource "aws_ecs_service" "nodejs_service_green" {
  name            = "nodejs-service"
  network_configuration {
    subnets = var.ecs_subnet_ids
    security_groups = var.ecs_security_group_ids
  ...
  cluster         = aws_ecs_cluster.ecs_cluster_green.id
  task_definition = aws_ecs_task_definition.nodejs_task_definition.arn
  desired_count   = 2
  launch_type     = "FARGATE"
  // iam_role        = aws_iam_role.foo.arn
  ...
  load_balancer {
    target_group_arn = var.nodejs_target_group_arn_green
    container_name   = "nodejs-container"
    container_port   = 3000
  }
